...
import type { ProjectWithProgress, Phase } from "../_types";
...
export function PhaseEditDialog({
  ...
  const handleOpenChange = useCallback(
    (newOpen: boolean) => {
      if (newOpen && project && phase) {
        ...
        setMemo(progressItem?.description || "");
      }
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      await onSave({ plannedDate, completedDate, memo });
      onOpenChange(false);
    } finally {
      setIsSaving(false);
    }
  }, [plannedDate, completedDate, memo, onSave, onOpenChange]);
